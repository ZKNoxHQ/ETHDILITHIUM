...
class TestModule(unittest.TestCase):
    ...
    def test_print(self):
        s = "Module over the commutative ring: Univariate Polynomial Ring in x over Finite Field of size 11^1 with modulus x^5 + 1"
...
class TestMatrix(unittest.TestCase):
    F = PrimeField(11)
    R = PolynomialRing(F, 5)
    R_prime = PolynomialRing(F, 2)
    M = Module(R)
    M_prime = Module(R_prime)
    F_dilithium = PrimeField(8380417)
    R_dilithium = PolynomialRing(F_dilithium, 256)
    ...
    def test_add_errors(self):
        A = self.M.random(2, 2)
        B = self.M.random(2, 3)
        A_prime = self.M_prime.random(2, 2)
    ...
    def test_matrix_dot(self):
        zero = self.R(0)
        for _ in range(100):
            u = [self.R.random() for _ in range(5)]
            v = [self.R.random() for _ in range(5)]
            dot = sum([ui * vi for ui, vi in zip(u, v)], zero)
