...
from polyntt.polynomial.polynomial_ntt import PolynomialRingNTT, PolynomialNTT


class PolynomialRingDilithium(PolynomialRingNTT):
    ...
    def rejection_sample_ntt_poly_32bit(self, rho, i, j, _xof=shake128):
        """
        Samples an element in the NTT domain of R^q using rejection sampling
        """
...
class PolynomialDilithium(PolynomialNTT):
    # def __init__(self, parent, coefficients, ntt=False):
    #     self.parent = parent
    #     self.coeffs = self._parse_coefficients(coefficients)

    def _sub_mod_q(self, a, b):
        return self.parent.F(a)-b
    ...
    def bit_pack_t0(self):
        # 416 = 256 * 13 // 8
        altered_coeffs = [self._sub_mod_q(1 << 12, c) for c in self.coeffs]
...
class PolynomialDilithiumNTT(PolynomialDilithium):
    ...
    #     if self.parent.q == 2**31-1:
    #         return [mul2(c1, c2) for c1, c2 in zip(f_coeffs, g_coeffs)]
    #     else:
