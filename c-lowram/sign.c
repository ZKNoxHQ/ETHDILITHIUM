...
int crypto_sign_keypair(uint8_t *pk, uint8_t *sk)
{
  ...
  printf("SEED:\n");
  for (int i = 0; i < SEEDBYTES + 2; i++)
  {
    printf("%02x", seedbuf[i]);
  }
  printf("\n");
#if ETHDILITHIUM
  inner_keccak256_prng_ctx prng;
  inner_keccak256_init(&prng);
  inner_keccak256_inject(&prng, seedbuf, SEEDBYTES + 2);
  inner_keccak256_flip(&prng);
  inner_keccak256_extract(&prng, seedbuf, 2 * SEEDBYTES + CRHBYTES);
#else
  shake256_init(s256);
...
#if ETHDILITHIUM
  inner_keccak256_init(&prng);
  inner_keccak256_inject(&prng, pk, CRYPTO_PUBLICKEYBYTES);
  inner_keccak256_flip(&prng);
  inner_keccak256_extract(&prng, tr, TRBYTES);
#else
...
#if ETHDILITHIUM
  inner_keccak256_init(&prng);
  inner_keccak256_inject(&prng, tr, TRBYTES);
  inner_keccak256_inject(&prng, mu, 2);
  inner_keccak256_inject(&prng, ctx, ctxlen);
  inner_keccak256_inject(&prng, m, mlen);
  inner_keccak256_flip(&prng);
  inner_keccak256_extract(&prng, mu, CRHBYTES);
...
#if ETHDILITHIUM
  inner_keccak256_init(&prng);
  inner_keccak256_inject(&prng, key, SEEDBYTES);
  inner_keccak256_inject(&prng, rnd, RNDBYTES);
  inner_keccak256_inject(&prng, mu, CRHBYTES);
  inner_keccak256_flip(&prng);
  /* rnd can be overwritten here */
  inner_keccak256_extract(&prng, rhoprime, CRHBYTES);
...
rej:
...
#if ETHDILITHIUM
    poly_uniform_gamma1_lowram_eth(tmp0, rhoprime, L * nonce + l_idx, &prng);
#else
...
#if ETHDILITHIUM
      poly_uniform_pointwise_montgomery_polywadd_lowram_eth(wcomp[k_idx], tmp0, rho, (k_idx << 8) + l_idx);
#else
  ...
  inner_keccak256_init(&prng);
  inner_keccak256_inject(&prng, mu, CRHBYTES);
  inner_keccak256_inject(&prng, sig, K * POLYW1_PACKEDBYTES);
  inner_keccak256_flip(&prng);
  inner_keccak256_extract(&prng, sig, CTILDEBYTES);
#else
  shake256_init(&state.s256);
  shake256_absorb(&state.s256, mu, CRHBYTES);
...
#if ETHDILITHIUM
    poly_uniform_gamma1_add_lowram_eth(tmp0, tmp0, rhoprime, L * (nonce - 1) + l_idx, &prng);
#else
  ...
  for (unsigned int k_idx = 0; k_idx < K; ++k_idx)
  {
    ...
    poly_schoolbook(tmp0, ccomp, sk + SEEDBYTES + TRBYTES + SEEDBYTES + L * POLYETA_PACKEDBYTES + K * POLYETA_PACKEDBYTES + k_idx * POLYT0_PACKEDBYTES);
                       ...
                       const uint8_t *pk)
{
  ...
  union
  {
    ...
    inner_keccak256_prng_ctx prng2;
    uint8_t c2[CTILDEBYTES];
  } xof_hint;
  uint8_t *hint_ones = xof_hint.hint_ones;
  inner_keccak256_prng_ctx *s128 = &xof_hint.prng2;
  uint8_t *c2 = xof_hint.c2;
#else
  keccak_state s256;
  union
  {
  ...
  inner_keccak256_inject(&prng, pk, CRYPTO_PUBLICKEYBYTES);
  inner_keccak256_flip(&prng);
  inner_keccak256_extract(&prng, mu, TRBYTES);

  inner_keccak256_init(&prng);
  inner_keccak256_inject(&prng, mu, TRBYTES);
#else
  shake256_init(&s256);
  ...
  for (size_t k_idx = 0; k_idx < K; k_idx++)
  {
...
#if ETHDILITHIUM
    poly_uniform_pointwise_montgomery_polywadd_lowram_eth(wcomp, &p, pk, (k_idx << 8) + 0);
#else
    ...
    for (size_t l_idx = 1; l_idx < L; l_idx++)
    {
      ...
      poly_ntt(&p);
#if ETHDILITHIUM
      poly_uniform_pointwise_montgomery_polywadd_lowram_eth(wcomp, &p, pk, (k_idx << 8) + l_idx);
#else
...
#if ETHDILITHIUM
    inner_keccak256_inject(&prng, w1_packed, POLYW1_PACKEDBYTES);
#else
...
#if ETHDILITHIUM
  inner_keccak256_flip(&prng);
  inner_keccak256_extract(&prng, c2, CTILDEBYTES);
#else
  shake256_finalize(&s256);
