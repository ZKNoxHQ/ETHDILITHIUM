...
void poly_uniform_pointwise_montgomery_polywadd_lowram_eth(uint8_t wcomp[3 * N],
                                                           poly *b,
                                                           ...
                                                           uint16_t nonce)
{
  ...
  uint8_t nonce_bytes[2] = {(uint8_t)(nonce & 0xFF),
                            (uint8_t)(nonce >> 8)};
  inner_keccak256_inject(&prng, nonce_bytes, 2);
  inner_keccak256_flip(&prng);

  size_t ctr = 0;
  do
  {
    /* Generate POLY_UNIFORM_BUFFERSIZE*3 pseudorandom bytes */
    inner_keccak256_extract(&prng, buf, sizeof(buf));

    for (size_t pos = 0; pos + 2 < sizeof(buf) && ctr < N; pos += 3)
    {
      /* reconstruct 23-bit integer from 3 bytes */
      t = buf[pos];
      t |= (uint32_t)buf[pos + 1] << 8;
      t |= (uint32_t)buf[pos + 2] << 16;
      t &= 0x7FFFFF;
...
void poly_uniform_gamma1_lowram_eth(poly *a,
                                    const uint8_t *seed,
                                    uint16_t nonce,
                                    inner_keccak256_prng_ctx *prng)
{
  ...
  for (size_t i = 0; i < N / POLY_UNIFORM_GAMMA1_BUFFERSIZE_COEFFS; i++)
  {
    inner_keccak256_extract(prng,
                            (uint8_t *)buf,
                            POLY_UNIFORM_GAMMA1_BUFFERSIZE_BYTES);
    polyz_unpack_inplace(buf);
...
void poly_uniform_gamma1_add_lowram_eth(poly *a,
                                        poly *b,
                                        ...
                                        inner_keccak256_prng_ctx *prng)
{
  ...
  size_t loops = N / POLY_UNIFORM_GAMMA1_BUFFERSIZE_COEFFS;
  for (size_t i = 0; i < loops; i++)
  {
    /* extract bytes into buf (cast to uint8_t*) */
    inner_keccak256_extract(prng,
                            (uint8_t *)buf,
                            POLY_UNIFORM_GAMMA1_BUFFERSIZE_BYTES);

    /* unpack in-place, exactly like original function */
    polyz_unpack_inplace(buf);

    /* add b->coeffs and store in a->coeffs */
    for (size_t j = 0; j < POLY_UNIFORM_GAMMA1_BUFFERSIZE_COEFFS; j++)
    {
      a->coeffs[i * POLY_UNIFORM_GAMMA1_BUFFERSIZE_COEFFS + j] =
          buf[j] + b->coeffs[i * POLY_UNIFORM_GAMMA1_BUFFERSIZE_COEFFS + j];
    }
  }
}
