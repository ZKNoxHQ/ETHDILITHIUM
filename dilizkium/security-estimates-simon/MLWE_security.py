...
class MLWEParameterSet:
    def __init__(self, n, d, m, k, q, distr="binomial"):
        ...
        self.q = q          # Modulus
        # Type of distribution : binomial or uniform (parameter k)
        self.distr = distr
...
def LWE_dual_cost(q, n, m, s, b, cost_svp=svp_classical, verbose=False):
    ...
    if verbose:
        ...
        print("shortest vector used has length l=%.2f, q=%d, `l<q'= %d" %
              (l, q, l < q))
        print("log2(epsilon) = %.2f, log2 nvector per run %.2f" %
...
def MLWE_optimize_attack(q, n, max_m, s, cost_attack=LWE_primal_cost, cost_svp=svp_classical, verbose=True):
    ...
    cost_attack(q, n, best_m, s, best_b,
                cost_svp=svp_classical, verbose=verbose)
...
def MLWE_summarize_attacks(ps):
    ...
    (m_pq, b_pq, c_pq) = MLWE_optimize_attack(q, n, max_m, s,
    ...
    print("Primal & %d & %d & %d & %d & %d" %
          (m_pq, b_pq, int(floor(c_pc)), int(floor(c_pq)), int(floor(c_pp))))

    (m_pc, b_pc, c_pc) = MLWE_optimize_attack(q, n, max_m, s,
                                              cost_attack=LWE_dual_cost, cost_svp=svp_classical, verbose=True)
    (m_pq, b_pq, c_pq) = MLWE_optimize_attack(q, n, max_m, s,
                                              cost_attack=LWE_dual_cost, cost_svp=svp_quantum, verbose=False)
    (m_pp, b_pp, c_pp) = MLWE_optimize_attack(q, n, max_m, s,
                                              cost_attack=LWE_dual_cost, cost_svp=svp_plausible, verbose=False)

    check_eq(m_pc, m_pq, m_pp)
    check_eq(b_pc, b_pq, b_pp)

    print("Dual & %d & %d & %d & %d & %d " %
          (m_pq, b_pq, int(floor(c_pc)), int(floor(c_pq)), int(floor(c_pp))))
